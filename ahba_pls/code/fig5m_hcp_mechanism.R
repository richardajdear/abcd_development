...
# Derived from fig5_hcp_summary.R; one PNG per AHBA matrix; panel c shows only the matched
...
# d: BH q; e: p; f: p; g: spin p of the feature map vs thinning rate).

suppressMessages({library(ggplot2); library(dplyr); library(tidyr); library(patchwork); library(scales)})

# Usage: Rscript code/fig5m_hcp_mechanism.R [3d_ds5|ds5|3d|base]   (AHBA matrix, see 22_hcp_summary.py)
VARIANT <- commandArgs(trailingOnly = TRUE)[1]; if (is.na(VARIANT)) VARIANT <- "base"
...
stopifnot(!any(is.na(e$set)), !any(is.na(e$vector)), nrow(e) == length(SET_ORD) * length(VORD))
pe <- ggplot(e, aes(vector, set)) +
  ...
  base_theme + theme(axis.line = element_blank(), axis.ticks = element_blank(),
                     axis.text.x.top = element_text(angle = 40, hjust = 0, vjust = 0, size = BASE - 1),
...
# ---------------------------------------------- e: MAGMA (traits as rows) --------
VEC <- c(ABCD_PLS1_HCP = "PLS1", AHBA_C1 = "AHBA C1", ABCD_PLS2_HCP = "PLS2", AHBA_C3 = "AHBA C3")
GA <- c(SCZ25_META = "Schizophrenia", MDD_div = "Depression", BIP = "Bipolar disorder", ADHD = "ADHD", ASD = "Autism",
        ALZ = "Alzheimer's disease", EA = "Educational attainment", INT = "Intelligence", HEIGHT = "Height")
GA_SHORT <- c(SCZ25_META = "SCZ", MDD_div = "MDD", BIP = "bipolar", ADHD = "ADHD", ASD = "ASD", ALZ = "Alzheimer's",
              EA = "EA", INT = "intelligence", HEIGHT = "height")
GGRP <- c(SCZ25_META = "psychiatric", MDD_div = "psychiatric", BIP = "psychiatric", ADHD = "psychiatric", ASD = "psychiatric",
          ALZ = "neuro-\ndegenerative", EA = "cognitive", INT = "cognitive", HEIGHT = "control")
GA <- GA[names(GA) %in% MG$gene_analysis]
...
cat(sprintf("[%s] traits: %s | global slope: min p = %.3f over %d tests (not plotted)\n",
            VARIANT, paste(names(GA), collapse = ","), min(slope$P), nrow(slope)))
pd <- ggplot(d, aes(BETA_STD, g, alpha = sig)) +
  ...
  labs(x = "MAGMA gene-property \u03b2 (standardised, 95% CI)", y = NULL, title = "e  Gene-level genetic risk (MAGMA)") +
...
# --------------------------------------- f: thirds of cortex + later symptoms ----
# thirds of the NORMATIVE thinning map (26_gradient_scores.py; the same for every
# AHBA matrix). Symptom rows are the two outcomes that line up with Figure 1.
TIERCOL <- c(slow = "#2166ac", mid = "grey55", fast = "#b2182b")
TIERLAB <- c(slow = "slow-thinning third", mid = "middle third", fast = "fast-thinning third")
TP  <- read.delim(file.path(RES, sprintf("tier_profile_%s.tsv", VARIANT)))
GT3 <- read.csv(file.path(RES, "gradient_tiers.csv"))
DC  <- read.delim(file.path(RES, "gradient_scores_decomposition.tsv"))
stopifnot(all(TP$variant == VARIANT))
um  <- GT3 |> group_by(tier) |> summarise(r = mean(normative_thinning_um_yr), .groups = "drop")
tl  <- setNames(sprintf("%s (%.0f \u00b5m/yr)", TIERLAB[um$tier], um$r), um$tier)
...
pf1 <- ggplot(tb, aes(x, y, group = interaction(view, label, group, subgroup), fill = tier)) +
  ...
  labs(title = "f  Rising symptoms go with faster thinning in slow cortex") + theme_void(base_size = BASE) +
  theme(plot.title = element_text(size = BASE, face = "bold", margin = margin(b = 3)),
        ...
        legend.key.size = unit(7, "pt"), legend.margin = margin(0, 0, 0, 0), plot.margin = margin(2, 4, 0, 4))
...
XR <- max(g$hi) * 1.08
pf2 <- ggplot(g, aes(y_sd_beta, o, colour = tier, alpha = sig)) +
  geom_vline(xintercept = 0, colour = "grey60", linewidth = 0.3) +
  ...
  coord_cartesian(xlim = c(min(g$lo), XR * 1.75), clip = "off") +
  labs(x = "\u03b2 per SD of thinning rate in that third\n(SD of CBCL at 15\u201317 | baseline CBCL)", y = NULL) +
  base_theme + theme(axis.line.y = element_blank(), axis.ticks.y = element_blank(), plot.margin = margin(4, 4, 2, 4))
pF <- (pf1 / pf2) + plot_layout(heights = c(0.62, 1))

# ------------------------------------------ g: what each third is made of ------
# features are parcel maps of mean z-expression (27_tier_profile.py); tier means
# are descriptive; fading follows the spin p of each feature map vs thinning rate
FEAT <- c(PLS2 = "PLS2 score", C3 = "AHBA C3 score",
          `Neuro-Ex` = "excitatory neurons", `Neuro-In` = "inhibitory neurons", Astro = "astrocytes",
          Micro = "microglia", Oligo = "oligodendrocytes", OPC = "OPCs",
          L1 = "layer 1", L2 = "layer 2", L3 = "layer 3", L4 = "layer 4", L5 = "layer 5", L6 = "layer 6", WM = "white matter",
          `SCZ top 500` = "SCZ top-500 genes", `MDD top 500` = "MDD top-500 genes")
FGRP <- c(PLS2 = "axis", C3 = "axis", `Neuro-Ex` = "cell classes", `Neuro-In` = "cell classes", Astro = "cell classes",
          Micro = "cell classes", Oligo = "cell classes", OPC = "cell classes",
          L1 = "layers", L2 = "layers", L3 = "layers", L4 = "layers", L5 = "layers", L6 = "layers", WM = "layers",
          `SCZ top 500` = "risk genes", `MDD top 500` = "risk genes")
h <- TP |> filter(feature %in% names(FEAT)) |>
  ...
  mutate(tier = factor(sub("mean_", "", tier), c("slow", "mid", "fast")), xi = as.integer(tier),
         f = factor(FEAT[feature], rev(FEAT)), grp = factor(FGRP[feature], c("axis", "cell classes", "layers", "risk genes")),
         sig = p_spin < 0.05)
stopifnot(nrow(h) == 3 * length(FEAT))
hr <- h |> distinct(f, grp, rho_with_thinning, p_spin, sig) |>
  mutate(txt = sprintf("%+.2f", rho_with_thinning),
         ptxt = ifelse(p_spin < 0.001, "< .001", sub("^0", "", sprintf("%.3f", p_spin))))
ML <- 0.8
hdr <- data.frame(grp = factor("axis", levels(h$grp)), x = c(4.0, 4.75), lab = c("\u03c1 with\nthinning rate", "p_spin"))
pG <- ggplot(h, aes(xi, f)) +
  geom_tile(aes(fill = pmax(pmin(m, ML), -ML), alpha = sig), colour = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%+.2f", m), alpha = sig), size = GT + 0.2) +
  geom_text(data = hr, aes(x = 4.0, label = txt, alpha = sig), size = GT + 0.2) +
  geom_text(data = hr, aes(x = 4.75, label = ptxt, alpha = sig), size = GT + 0.2) +
  geom_text(data = hdr, aes(x = x, y = Inf, label = lab), inherit.aes = FALSE, vjust = -0.25, size = GT,
            fontface = "bold", lineheight = 0.9) +
  scale_x_continuous(breaks = 1:3, labels = c("slow", "middle", "fast"), limits = c(0.5, 5.2), expand = c(0, 0), position = "top") +
  ...
  labs(x = NULL, y = NULL, title = "g  What each third of normative thinning is made of") +
  base_theme + theme(axis.line = element_blank(), axis.ticks = element_blank(),
                     axis.text.x.top = element_text(size = BASE - 0.5, face = "bold", colour = "grey20"),
                     strip.placement = "outside", strip.text.y.left = element_text(angle = 0, hjust = 1, size = BASE - 1),
                     panel.spacing.y = unit(4, "pt"), legend.key.height = unit(16, "pt"), legend.key.width = unit(6, "pt"),
                     plot.title = element_text(margin = margin(b = 16)))

# -------------------------------------------- claims for the figure subtitle ----
# computed from the plotted tables, because four AHBA matrices are rendered
s_sig <- TP |> filter(feature %in% c("Neuro-Ex", "Neuro-In", "Oligo", "Astro", "Micro", "L2", "L3", "WM"), p_spin < 0.05)
nm <- c(`Neuro-Ex` = "excitatory-neuron", `Neuro-In` = "inhibitory-neuron", Oligo = "oligodendrocyte", Astro = "astrocyte",
        Micro = "microglial", L2 = "layer-2", L3 = "layer-3", WM = "white-matter")
fewer <- nm[s_sig$feature[s_sig$mean_slow < 0]]; more <- nm[s_sig$feature[s_sig$mean_slow > 0]]
compo <- paste(c(if (length(fewer)) paste("fewer", paste(fewer, collapse = " / "), "genes"),
...
sub_txt <- sprintf(paste0(
  "a\u2013e: PLS of AHBA expression (%s: %d parcels \u00d7 %s genes) against ABCD baseline thickness and thinning; PLS2 = AHBA C3. PLS2-aligned genes carry more common-variant signal for %s (e, p < 0.05).\n",
  "f\u2013g: children whose symptoms rise over adolescence thin faster in the slowest-thinning third of cortex (f), which sits at the PLS2-low pole (PLS2 %+.2f SD; %s; g).\n",
  "%s"),
...
row2 <- (pc1 | pc2 | pe) + plot_layout(widths = c(0.9, 0.9, 0.85))
row3 <- (pd | pF | pG) + plot_layout(widths = c(1.25, 0.95, 1.05))
# wrap each row so axes are NOT aligned across rows (panel e's long trait labels
# would otherwise push panel c's plotting area to the right)
...
fig <- fig + plot_annotation(
    title = "The thinning axis (PLS2 = AHBA C3) carries psychiatric gene-level risk; children whose symptoms rise thin faster in normally slow, PLS2-low cortex",
    subtitle = sub_txt,
...
OUT <- file.path(FIG, sprintf("fig_hcp_mechanism_%s.png", VARIANT))
ggsave(OUT, fig, width = 12, height = 10.4, dpi = 300, bg = "white")
cat("wrote", OUT, "\n")
