...
the imaging-transcriptomics route with a 20× larger discovery sample, do we recover the
...
Bilateral (LH/RH-averaged) DK maps from the settled ABCD 7.0 mixed-model runs.
...
`dCT` and `CT` reproduce `docs/developmental_maps_noglobal.csv` to <1e-5. One T1w/T2w fit
(`rh_temporalpole`) is singular and is kept with a flag (`data/Y_MAPS.md`).
...
(p = 0.06–0.10) and whose gene vector loads as heavily on C1, the dominant static
expression gradient, as on C3 (ρ = 0.59 with each). That is the interpretive trap: the
...
Options 3 and 4 find the same component (pairwise gene-weight ρ 0.83–0.94 with option 2),
and the signature is essentially invariant to the DS filter (ρ ≥ 0.99 across ds0/ds25/ds50) —
...
that isolates the signature (SCZ β_std 0.028–0.041, MDD 0.024–0.041), at roughly **half
...
(z = 2.4 at ds25). Importantly, the SCZ locus-pool signal that *is* present sits on the
static PLS1 and on C1 (z ≈ 2.1–2.3) — so it is a property of the dominant expression
...
survives the joint model (ABCD p = 0.36, NSPN p = 0.085, from 0.014 and 0.004 alone),
while for MDD only the ABCD signature retains its association (p = 0.015 vs 0.87).

## What this says

**The replication is the result.** A discovery sample 20× larger, a different scanner
...
**Open questions** are specified for the cluster in `FOLLOWUP_GENETICS.md`: whether the
signature weights relate to the genes driving ABCD's own global thinning (H3; the prior
from `hpc_v3` is null), and whether projecting subject-level slopes onto the component
gives a more heritable GWAS phenotype than `global_slope` (H4; benchmarks h² = 0.115 ± 0.056
and `slope_projC3` 0.121 ± 0.057, so a large gain would be surprising).
...
Rscript code/fig1_lead_signature.R        # Figure 1  (ggplot2 + patchwork + ggseg)
Rscript code/fig2_enrichment.R            # Figure 2
```
...
scripts read only saved tables and recompute nothing, so every number printed on a figure
comes from the file it is plotting; DK polygon coordinates are cached once to
`data/dk_polygons.csv` from the ggseg atlas. MAGMA v1.10 binaries and their provenance are
