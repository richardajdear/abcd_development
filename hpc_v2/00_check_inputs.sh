...
check_bfile() {
  ...
  if [[ "$magic" != "6c1b01" ]]; then
    ...
    bad "$prefix.bed is $size bytes, expected $expect (n=$n, m=$m) -- bim/fam/bed mismatch, see hpc/README_HPC.md §5.1"
  else
...
if "$RSCRIPT" -e 'suppressMessages({library(GENESIS); library(SNPRelate); library(GWASTools)})' >/dev/null 2>&1; then
  ...
  bad "RSCRIPT=$RSCRIPT cannot load GENESIS/SNPRelate/GWASTools -- build the env from hpc_v2/envs/genesis_env.yml"
fi
