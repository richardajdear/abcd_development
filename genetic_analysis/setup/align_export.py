...
found by comparing its output against ``legacy/hpc/work/pheno_allanc_prs/`` (the
...
2. **Scale.**  v1 z-scored every phenotype column
   (``legacy/hpc/work/standardise_pheno.py``) because ``global_slope`` has
...
same covariates, same PCs, same order as every v2 result.
    ...
    python genetic_analysis/setup/align_export.py <src-gcta_inputs_v3> <v2-pheno-dir> <out-dir>
"""
