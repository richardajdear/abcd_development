# Shared configuration for the genetic_analysis/ pipeline.  Sourced by every
# script; never run directly.
#
# This is the v2 (legacy/hpc_v2) GENESIS pipeline -- KING -> PC-AiR -> PC-Relate
# kinship, mixed-model association that KEEPS relatives, within-family PRS --
# re-pointed at the phenotypes from the true 7.0 tabulation (2026-09-14).  The
# genotype side (GDS files, kinship, PRS score profiles) is phenotype-independent
# and is REUSED from the v2 tree; only phenotype-dependent steps write here.
# See README_HPC.md for the run order.
#
# DESIGN RULE (inherited, do not regress): no path is hard-coded.  Every one is
# `${VAR:-default}`; genetic_analysis/config.local.sh (gitignored) is sourced
# FIRST so a root set there propagates through the derived defaults.
...
if [[ -f "$REPO_ROOT/genetic_analysis/config.local.sh" ]]; then
  # shellcheck source=/dev/null
  source "$REPO_ROOT/genetic_analysis/config.local.sh"
fi
...
# 7.0-tabulation export (FID = family_id; see README_HPC.md step 1).  The
# 6.0-vintage exports every legacy number ran on are $ABCD_HPC_ROOT/pheno and
# legacy/hpc/work/pheno_allanc_prs/ -- never point this at them.
PHENO_DIR="${PHENO_DIR:-$ABCD_HPC_ROOT/pheno_70tab}"
...
# behind legacy/hpc/README_HPC.md §8.15 in config.local.sh.
...
# ---------------------------------------------------------------------------
# New outputs.  results_v2/ holds every legacy v2 result and is read-only from here.
OUT_V2="${OUT_V2:-$ABCD_HPC_ROOT/results_70tab}"
V2_LEGACY="${V2_LEGACY:-$ABCD_HPC_ROOT/results_v2}"
# Genotype-only products: reuse v2's, do not rebuild (README_HPC.md sec 3).
GDS_DIR="${GDS_DIR:-$V2_LEGACY/gds}"
KIN_DIR="${KIN_DIR:-$V2_LEGACY/kinship}"
...
# On CSD3 build it once from genetic_analysis/envs/genesis_env.yml (see README) and set
...
# ---------------------------------------------------------------------------
# Helpers (same contracts as legacy/hpc/config.sh)
# ---------------------------------------------------------------------------
run() {
  if [[ "$DRY_RUN" == "1" ]]; then
...
require_paths() {
  ...
  [[ "$missing" == "0" ]] || {
    echo >&2
    echo "Set the variables above, or write $REPO_ROOT/genetic_analysis/config.local.sh." >&2
    exit 2
  }
}
