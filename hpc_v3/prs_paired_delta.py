...
nested scores as independent; `hpc_v2/README_HPC.md` section 7 records that the
...
and `hpc/README_HPC.md` section 2 already records that a PRS-SELECTED
...
built from the same BLUPs and pushed through the same regression.
    ...
    python hpc_v3/prs_paired_delta.py --out-dir hpc_v3/work/prs
"""
...
def minp_permutation(strata, scores, n_perm, seed) -> pd.DataFrame:
    ...
    ~4.1 effective tests, hpc_v2/README_HPC.md section 7); this is the right one.
    """
    rng = np.random.default_rng(seed)
    rows = []
    for st in strata:
...
def random_region_null(strata, scores, run_dir, disorders, thresholds,
    ...
    ask whether THIS subset beats an arbitrary one.  hpc/README_HPC.md section
...
def main() -> int:
    ...
    ap.add_argument("--pheno-dir", default=str(REPO / "hpc_v2/work/pheno_v3"))
    ap.add_argument("--prs-dir",
                    default=str(REPO / "hpc_v2/work/results_v2/prs_ct_v3"))
    ap.add_argument("--eur-ids",
                    default=str(REPO / "hpc/work/results/ancestry/eur_anchor.keep"))
    ap.add_argument("--run-dir",
                    default=str(REPO / "out/thickness_dsk_70_139406217085"))
    ap.add_argument("--out-dir", default=str(REPO / "hpc_v3/work/prs"))
